...
export load_graph_data, gen_rand_vertex_name, gen_vertex_hash, generate_synthetic_vertices, generate_synthetic_edges, insert_vertices_from_jl, copy_input_graph_to_new_graph, insert_edges_from_jl, main
...
function insert_vertices_from_jl(vertices_file::String, input_graph::Nothing)
    ...
    # The graph is currently empty; objective is to create it from the input vertex and edge files.
    ...
    n = nv(G)
    @info("Returning graph G; G contains $n vertices.")
    return G
...
function insert_vertices_from_jl(vertices_file::String, input_graph::MetaDiGraph)
    ...
    for v in eachrow(v_df)
        ...
        try
            ...
            @info("A vertex with this hash value already exists in the graph.")
    ...
    @info("Returning graph G; G now contains $n_vertices_final vertices.")
    return G
...
function insert_edges_from_jl(edges_file::String, input_graph::MetaDiGraph)
    ...
    for e in eachrow(e_df)
        ...
        if !(e.src_vhash in vertices_already_in_G)
            ...
            vname = e.src_name
            @info("src vertex $vname was not in G, and has been inserted.")
        end
        ...
        if !(e.dst_vhash in vertices_already_in_G)
            ...
            vname = e.dst_name
            @info("dst vertex $vname was not in G, and has been inserted.")
        ...
        #  This edge already exists \in G; increment the (frequency) counter (currently used for weight; we will probably want to think about how to best normalize these counts)
            ...
            @info("Incrementing weight of existing directed edge of type $edge_type from $src_name to $dst_name. \n Weight is now: $new_weight.")
